...
4. Always respond to the user in ${language}.
...
export const translateHistoryNode = async (
  ...
  const prompt = `Translate the following conversation history into ${language}.
Maintain the original roles (user/assistant).
Do NOT modify numbers, dates, or currency symbols (₪ / ש"ח).
Return ONLY a valid JSON array of objects with keys 'role' and 'content'.
...
${JSON.stringify(messages)}`;
  ...
  try {
    const translatedMessages = JSON.parse(response.content as string);
